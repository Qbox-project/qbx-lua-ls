...
server.stdout.on('data', (chunk) => {
    ...
    for (;;) {
        ...
        else if (message.method === 'textDocument/publishDiagnostics') diagnostics = message.params.diagnostics;
    }
});
let diagnostics = [];
...
const uri = pathToFileURL(resolve(virtualFile)).href;
...
console.log('diagnostics:', diagnostics.map((d) => `${d.range.start.line + 1}:${d.code}`).join(', ') || 'none');
